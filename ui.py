...
class StartupDetectorUI:
    def __init__(self, root):
        self.root = root
        self.root.withdraw()  # Evita piscada inicial
    ...
    def configurar_estilo(self):
        ...
        self.style.configure("TNotebook.Tab", background=self.tema_atual["panel"],
                             foreground=self.tema_atual["text"], padding=(12, 6),
    ...
    def criar_interface(self):
        ...
        ttk.Label(topo, text="Gerenciador", font=("Segoe UI", 16, "bold"),
                  foreground=self.tema_atual["accent"]).pack(side="left")
        ...
        self.label_saude = ttk.Label(topo, text="🟢 Saúde: Boa", font=("Segoe UI", 11, "bold"),
                                     style="SaudeBoa.TLabel")
        ...
        self.criar_aba_recursos()

        # ===== RODAPÉ =====
        rodape = ttk.Frame(self.root)
        rodape.pack(side="bottom", fill="x")
        ttk.Label(rodape, text="Desenvolvido por Daniel Marques • Projeto acadêmico — Python + Tkinter",
                  font=("Segoe UI", 9), anchor="center").pack(pady=4)
    ...
    def criar_aba_programas(self, aba, funcao):
        ...
        lista = tk.Listbox(frame, bg=self.tema_atual["panel"], fg=self.tema_atual["text"],
                           selectbackground=self.tema_atual["accent"], highlightthickness=0, font=("Segoe UI", 10))
    ...
    # ---------------- ABA RECURSOS ----------------
    def criar_aba_recursos(self):
        frame = ttk.Frame(self.aba_recursos)
        ...
        def atualizar_recursos():
            ...
            for i, prog in enumerate(programas):
                ...
                # CPU
                cpu = prog.get("cpu", 0)
                style_cpu_name = f"CPUBar{i}.Horizontal.TProgressbar"
                style_cpu = ttk.Style()
                style_cpu.configure(style_cpu_name, troughcolor=self.tema_atual["panel"], background=self._cor_cpu(cpu))
                cpu_bar = ttk.Progressbar(prog_frame, maximum=100, value=cpu, style=style_cpu_name)
                cpu_bar.pack(fill="x", pady=2)
                ttk.Label(prog_frame, text=f"CPU: {cpu}%").pack(anchor="w")

                # RAM
                ram = prog.get("ram", 0)
                ram_percent = round((ram / total_ram) * 100, 1)
                style_ram_name = f"RAMBar{i}.Horizontal.TProgressbar"
                style_ram = ttk.Style()
                style_ram.configure(style_ram_name, troughcolor=self.tema_atual["panel"], background=self._cor_ram(ram_percent))
                ram_bar = ttk.Progressbar(prog_frame, maximum=100, value=ram_percent, style=style_ram_name)
                ram_bar.pack(fill="x", pady=2)
                ttk.Label(prog_frame, text=f"RAM: {ram} MB ({ram_percent}%)").pack(anchor="w")
    ...
    # ---------------- CORES DINÂMICAS ----------------
    def _cor_cpu(self, valor):
        if valor < 50:
            return "#4CAF50"  # verde
        elif valor < 80:
            return "#FFB300"  # amarelo
        else:
            return "#D32F2F"  # vermelho

    def _cor_ram(self, valor):
        if valor < 50:
            return "#4CAF50"  # verde
        elif valor < 80:
            return "#FFB300"  # amarelo
